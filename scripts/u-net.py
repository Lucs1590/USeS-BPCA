...
def get_unet_model(img_size, num_classes):
    ...
    concatenating_layers = []
    for filters in [64, 128, 256, 512]:
        x = layers.Conv2D(filters, activation='relu', kernel_size=3)(x)
        x = layers.Conv2D(filters, activation='relu', kernel_size=3)(x)
        concatenating_layers.append(x)
        x = layers.MaxPool2D(pool_size=(2, 2), strides=(2, 2), padding='valid')(x)

    x = layers.Conv2D(1024, activation='relu', kernel_size=3)(x)
    x = layers.Conv2D(1024, activation='relu', kernel_size=3, padding='valid')(x)

    for filters in [512, 256, 128]:
        transp = layers.Conv2DTranspose(filters, kernel_size=2, strides=(2, 2), activation='relu')(x)
        conc_conv = concatenating_layers.pop()
        crop = layers.Cropping2D(cropping=define_crop_values(filters))(conc_conv)
